...
public abstract class PixelmonBoosterFactory implements IModuleFactory<PixelmonBoosterFactory> {
	public static PixelmonBoosterFactory create(IModuleFactory<? extends JavaPlugin> plugin) {
		return (PixelmonBoosterFactory) ReflectionUtil.getConstructor("dev.rachamon.rachamoncore.compatible." + MCVersion.getCurrent()
						.name() + ".pixelmonbooster.factory.PixelmonBoosterFactoryImpl", IModuleFactory.class)
				.invoke(plugin);
	}
