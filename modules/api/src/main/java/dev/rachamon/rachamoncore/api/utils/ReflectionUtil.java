...
public class ReflectionUtil {
	...
	private static <T> FieldAccessor<T> getField(Class<?> target, String name, Class<T> fieldType, int index) {
		for (final Field field : target.getDeclaredFields()) {
			if ((name == null || field.getName()
					.equals(name)) && (fieldType == null || fieldType.isAssignableFrom(field.getType())) && index-- <= 0) {
	...
	public static MethodInvoker<?> getTypedMethod(
		...
		List<Method> sortedMethodsByArgNumber = Arrays.stream(clazz.getDeclaredMethods())
		...
		for (final Method method : sortedMethodsByArgNumber) {
			if ((methodName == null || method.getName()
					.equals(methodName)) && (returnType == null || method.getReturnType()
					.equals(returnType)) && (params.length == 0 || Arrays.equals(method.getParameterTypes(), params))) {
				...
				return new MethodInvoker<Object>() {
					...
					public Object invoke(Object target, Object... arguments) {
						...
						for (int i = 0; i < arguments.length; i++) {
							...
							correctArgs[i] = arguments[i] != null && arguments[i].getClass()
									.isArray() ? new Object[]{arguments[i]} : arguments[i];
						}
						try {
		...
		throw new IllegalStateException(String.format("Unable to find method %s(%s).",
				methodName,
				params.length == 0 ? "" : Arrays.asList(params)
		));
	}
	...
	public static <T> ConstructorInvoker<?> getConstructor(Class<T> clazz, Class<?>... params) {
		...
		for (Constructor<?> declaredConstructor : clazz.getDeclaredConstructors()) {
			possibleConstructors.append("\n")
					...
					.append("(")
					.append(Arrays.toString(declaredConstructor.getParameterTypes()))
					.append(")");
		}
	...
	public static boolean arrayEqualsIgnorePrimitives(Class<?>[] array1, Class<?>[] array2) {
		...
		for (int i = 0; i < array1.length; i++) {
			...
			try {
				...
				Object nonPrimitiveInnerType = clazz1.isPrimitive() ? ReflectionUtil.getField(clazz2, "TYPE")
						.get(null) : ReflectionUtil.getField(clazz1, "TYPE").get(null);
