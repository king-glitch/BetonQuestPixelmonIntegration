...
public class CommandManager implements CommandExecutor, TabCompleter {
	...
	private final List<String> syntaxError = Arrays.asList(
	...
	public boolean onCommand(
			@NotNull CommandSender commandSender, Command command, @NotNull String label, String[] args
	) {
	...
	public List<String> onTabComplete(
		...
		if (nested != null) {
			...
			if (args.length == 1) {
				...
				return nested.children.entrySet()
						.stream()
			...
			String subCmd = getSubCommand(nested, args);
			AbstractCommand sub;
			if (subCmd != null && (sub = nested.children.get(subCmd)) != null && (!console || !sub.isNoConsole()) && (op || sub.getPermissionNode() == null || sender.hasPermission(
					sub.getPermissionNode()))) {
				// adjust the arguments to match - BREAKING!!
				int i = subCmd.indexOf(' ') == -1 ? 1 : 2;
				String[] newArgs = new String[args.length - i];
				System.arraycopy(args, i, newArgs, 0, newArgs.length);
