...
public abstract class PaletteTokensFactory implements IModuleFactory<PaletteTokensFactory> {
	...
	public static PaletteTokensFactory create(IModuleFactory<? extends JavaPlugin> plugin) {
		return (PaletteTokensFactory) ReflectionUtil.getConstructor("dev.rachamon.rachamoncore.compatible." + MCVersion.getCurrent()
				.name() + ".palletetokens.factory.PaletteTokensFactoryImpl", IModuleFactory.class).invoke(plugin);
	}
