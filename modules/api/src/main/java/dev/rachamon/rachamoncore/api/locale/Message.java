		...
		if (message.matches(".*\\{#[0-9a-fA-F]{6}}.*")) {
			...
			if (MCVersion.isLower(MCDetailedVersion.v1_16_R1)) {
				...
				int green = Integer.parseInt(
						message.substring(message.indexOf("{#") + 4, message.indexOf("{#") + 6),
						16
				);
				int blue = Integer.parseInt(
						message.substring(message.indexOf("{#") + 6, message.indexOf("{#") + 8),
						16
				);

				message = message.replaceAll(
						"\\{#[0-9a-fA-F]{6}}",
						"&x" + ColorUtil.fromRGB(red, green, blue).getChatColor().toString()
				);
			} else {
